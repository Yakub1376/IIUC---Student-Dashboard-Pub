...
import com.blogspot.softlabsja.iiucstudentapp.RegistrationSummary.RegistrationSummaryAdapter;
import com.blogspot.softlabsja.iiucstudentapp.RegistrationSummary.RegistrationSummaryModel;
...
public class CourseRegistrationAdapter extends RecyclerView.Adapter<CourseRegistrationAdapter.ViewHolder> {
    ArrayList<CourseRegistrationModel> courseRegistrationModels;
    ...
    public CourseRegistrationAdapter(ArrayList<CourseRegistrationModel> courseRegistrationModels, ArrayList<String> CourseSubOptionModel, Context context,
                                     TextView h_1, TextView h_2, TextView h_3, TextView h_4,
                                     TextView h_5, TextView h_6, String selected, Button submitBtn,
        ...
        this.courseRegistrationModels = courseRegistrationModels;
    ...
    @NonNull
    @Override
    public CourseRegistrationAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {


        LayoutInflater inflater = LayoutInflater.from(mContext);
        View contactView = inflater.inflate(R.layout.course_registration_item, parent, false);

        ViewHolder myViewHolder = new ViewHolder(contactView);

        for (CourseRegistrationModel m : courseRegistrationModels) {
    ...
    @Override
    public void onBindViewHolder(@NonNull CourseRegistrationAdapter.ViewHolder holder, int position) {
        ...
        if (!courseRegistrationModels.get(position).getCourseState().equals("0")) {
            s = courseRegistrationModels.get(position).getCourseState();
            int f = 0;
        ...
        int spinnerPosition = langAdapter.getPosition(selected);
        ...
        holder.spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                String s1 = "course_id[]";
                String checkboxValue = courseRegistrationModels.get(poo2).getCheckboxValue().select("input").attr("value");

                String s2 = "credit_hours_" + checkboxValue;
                String creditHoursValue = courseRegistrationModels.get(poo2).getCredit_Hours().select("input").attr("value");
        ...
        Element checkboxValue = courseRegistrationModels.get(position).getCheckboxValue();
        Element creditHours = courseRegistrationModels.get(position).getCredit_Hours();


        holder.i_1.setText(checkboxValue.text());
        holder.i_2.setText(courseRegistrationModels.get(position).getCourse_Code());
        holder.i_3.setText(courseRegistrationModels.get(position).getCourse_Name());
        holder.i_4.setText(creditHours.text());
        holder.i_5.setText(courseRegistrationModels.get(position).getSection());
        ...
        holder.tableRow.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                courseRegistrationModels.get(position).setSelected(!courseRegistrationModels.get(position).isSelected());
                if (courseRegistrationModels.get(position).isSelected()) {
                    ...
                    String s1 = "course_id[]";
                    String checkboxValue = courseRegistrationModels.get(position).getCheckboxValue().select("input").attr("value");

                    String s2 = "credit_hours_" + checkboxValue;
                    String creditHoursValue = courseRegistrationModels.get(position).getCredit_Hours().select("input").attr("value");
...
//                }
    ...
    public int getItemCount() {
        return courseRegistrationModels.size();
    }
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Document d = Jsoup.connect("https://www.iiuc.ac.bd/student/course-registration/" + student_id)
                    ...
                    if (!text2.contains(s1) && !text2.contains(s3) && !text2.contains(s2)) {
